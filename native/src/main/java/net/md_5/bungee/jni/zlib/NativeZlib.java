...
public class NativeZlib implements BungeeZlib {
    ...
    @Getter private final NativeCompressImpl nativeCompress = new NativeCompressImpl();
    /*============================================================================*/
    ...
    @Override public void process(ByteBuf in, ByteBuf out) throws DataFormatException {
        // Smoke tests
        in.memoryAddress();
        out.memoryAddress();
        ...
        while (!nativeCompress.finished && (compress || in.isReadable())) {
            ...
            int processed = nativeCompress
                .process(ctx, in.memoryAddress() + in.readerIndex(), in.readableBytes(),
                    out.memoryAddress() + out.writerIndex(), out.writableBytes(), compress);

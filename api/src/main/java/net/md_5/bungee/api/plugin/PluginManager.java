 ...
 */
@RequiredArgsConstructor public class PluginManager {
    ...
    @SuppressWarnings("unchecked") public PluginManager(ProxyServer proxy) {
        this.proxy = proxy;
        ...
        try {
            if (tabResults == null) {
                if (proxy.getConfig().isLogCommands()) {
                    proxy.getLogger().log(Level.INFO, "{0} executed command: /{1}",
            ...
            ProxyServer.getInstance().getLogger()
                .log(Level.WARNING, "Error in dispatching command", ex);
        }
        return true;
    }
    ...
    public void enablePlugins() {
        for (Plugin plugin : plugins.values()) {
            try {
                plugin.onEnable();
                ProxyServer.getInstance().getLogger()
                    .log(Level.INFO, "Enabled plugin {0} version {1} by {2}",
                        new Object[] {plugin.getDescription().getName(),
                            plugin.getDescription().getVersion(),
                            plugin.getDescription().getAuthor()});
            } catch (Throwable t) {
                ProxyServer.getInstance().getLogger().log(Level.WARNING,
                    "Exception encountered when loading plugin: " + plugin.getDescription()
                        .getName(), t);
            }
        }
    }
    ...
    private boolean enablePlugin(Map<PluginDescription, Boolean> pluginStatuses,
        ...
        for (String dependName : dependencies) {
            ...
            if (dependStatus == Boolean.FALSE && plugin.getDepends()
                .contains(dependName)) // only fail if this wasn't a soft dependency
            {
                ProxyServer.getInstance().getLogger()
        ...
        if (status) {
            try {
                URLClassLoader loader =
                    new PluginClassloader(new URL[] {plugin.getFile().toURI().toURL()});
                ...
                proxy.getLogger()
                    .log(Level.WARNING, "Error enabling plugin " + plugin.getName(), t);
            }
        }
            ...
            if (file.isFile() && file.getName().endsWith(".jar")) {
                try (JarFile jar = new JarFile(file)) {
                    ...
                    try (InputStream in = jar.getInputStream(pdf)) {
                        ...
                        Preconditions
                            .checkNotNull(desc.getName(), "Plugin from %s has no name", file);
                        Preconditions
                            .checkNotNull(desc.getMain(), "Plugin from %s has no main", file);
    ...
    public <T extends Event> T callEvent(T event) {
        ...
        if (elapsed > 250000000) {
            ProxyServer.getInstance().getLogger()
                .log(Level.WARNING, "Event {0} took {1}ns to process!",
                    new Object[] {event, elapsed});
        }
        return event;
    }

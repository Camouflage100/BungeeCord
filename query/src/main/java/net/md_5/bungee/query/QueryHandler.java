...
@RequiredArgsConstructor public class QueryHandler
    ...
    private ProxyServer bungee = null;
    ...
    @Override protected void channelRead0(ChannelHandlerContext ctx, DatagramPacket msg)
        ...
        if (type == 0x09) {
            ...
            sessions.put(msg.sender().getAddress(),
                new QuerySession(challengeToken, System.currentTimeMillis()));
        ...
        if (type == 0x00) {
            ...
            if (in.readableBytes() == 0) {
                ...
                // Long Response
                out.writeBytes(
                    new byte[] {0x73, 0x70, 0x6C, 0x69, 0x74, 0x6E, 0x75, 0x6D, 0x00, (byte) 0x80,
                        0x00});

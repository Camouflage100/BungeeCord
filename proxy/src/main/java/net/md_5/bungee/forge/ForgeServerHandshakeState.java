 ...
 */
public enum ForgeServerHandshakeState
    implements IForgeServerPacketHandler<ForgeServerHandshakeState> {
    ...
    START {
        ...
        @Override public ForgeServerHandshakeState send(PluginMessage message, UserConnection con) {
        ...
        @Override public ForgeServerHandshakeState send(PluginMessage message, UserConnection con) {
            if (message.getData()[0] == 3 && message.getTag()
                .equals(ForgeConstants.FML_HANDSHAKE_TAG)) {
                con.getForgeClientHandler().setServerIdList(message);
                return this;
            }

            if (message.getData()[0] == -1 && message.getTag().equals(
                ForgeConstants.FML_HANDSHAKE_TAG)) // transition to COMPLETE after sending ACK
            {
                return this;
            }

            if (message.getTag()
                .equals(ForgeConstants.FORGE_REGISTER)) // wait for Forge channel registration
            {
                return COMPLETE;
            }

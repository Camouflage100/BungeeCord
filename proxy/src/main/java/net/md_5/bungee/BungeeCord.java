...
public class BungeeCord extends ProxyServer {
    ...
    @SuppressFBWarnings("DM_DEFAULT_ENCODING") public BungeeCord() throws IOException {
        ...
        if (!Boolean.getBoolean("net.md_5.bungee.native.disable")) {
            if (EncryptionUtil.nativeFactory.load()) {
                ...
                logger.info(
                    "Using standard Java JCE cipher. To enable the OpenSSL based native cipher, please make sure you are using 64 bit Ubuntu or Debian with libssl installed.");
            }
            if (CompressFactory.zlib.load()) {
                logger.info("Using native code compressor");
            } else {
                logger.info(
                    "Using standard Java compressor. To enable zero copy compression, run on 64 bit Linux");
            }
        }
    }
     ...
     * @throws Exception
     */
    @Override @SuppressFBWarnings("RV_RETURN_VALUE_IGNORED_BAD_PRACTICE") public void start()
        throws Exception {
    ...
    public void startListeners() {
        for (final ListenerInfo info : config.getListeners()) {
            ChannelFutureListener listener = new ChannelFutureListener() {
                @Override public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        ...
                        getLogger().log(Level.WARNING, "Could not bind to host " + info.getHost(),
                            future.cause());
                    }
                }
            };
            new ServerBootstrap().channel(PipelineUtils.getServerChannel())
                ...
                .group(eventLoops).localAddress(info.getHost()).bind().addListener(listener);
            ...
            if (info.isQueryEnabled()) {
                ChannelFutureListener bindListener = new ChannelFutureListener() {
                    @Override public void operationComplete(ChannelFuture future) throws Exception {
                        if (future.isSuccess()) {
                            listeners.add(future.channel());
                            getLogger().log(Level.INFO, "Started query on {0}",
                                future.channel().localAddress());
                        } else {
                            getLogger()
                                .log(Level.WARNING, "Could not bind to host " + info.getHost(),
                                    future.cause());
                        }
                    }
                };
    ...
    @Override public void stop(final String reason) {
        new Thread("Shutdown Thread") {
            @Override @SuppressFBWarnings("DM_EXIT") @SuppressWarnings("TooBroadCatch")
            public void run() {
                ...
                try {
                    getLogger()
                        .log(Level.INFO, "Disconnecting {0} connections", connections.size());
    ...
    @Override public String getTranslation(String name, Object... args) {
        ...
        try {
            translation = MessageFormat.format(
                customBundle != null && customBundle.containsKey(name) ?
                    customBundle.getString(name) :
    ...
    @Override public Collection<String> getDisabledMods() {
        return config.getDisabledMods();
    }
    ...
    @Override public Collection<ProxiedPlayer> matchPlayer(final String partialName) {
        ...
        return Sets.newHashSet(getPlayers().stream().filter(
            input -> input != null && input.getName().toLowerCase()
                .startsWith(partialName.toLowerCase())).collect(Collectors.toList()));
    }

...
public class ModuleManager {
    ...
    public void load(ProxyServer proxy, File moduleDirectory) throws Exception {
        ...
        for (String s : (List<String>) config.get("modules")) {
            ...
            ModuleSpec spec =
                new ModuleSpec(name, new File(moduleDirectory, name + ".jar"), source);
            modules.add(spec);
        ...
        for (ModuleSpec module : modules) {
            ModuleVersion moduleVersion =
                (module.getFile().exists()) ? getVersion(module.getFile()) : null;

            if (!bungeeVersion.equals(moduleVersion)) {
                System.out.println(
                    "Attempting to update plugin from " + moduleVersion + " to " + bungeeVersion);
                module.getProvider().retrieve(module, bungeeVersion);
            }
        }
    }

    @SuppressFBWarnings("REC_CATCH_EXCEPTION") private ModuleVersion getVersion(File file) {
        try (JarFile jar = new JarFile(file)) {
            ...
            ProxyServer.getInstance().getLogger()
                .log(Level.WARNING, "Could not check module from file " + file, ex);
        }

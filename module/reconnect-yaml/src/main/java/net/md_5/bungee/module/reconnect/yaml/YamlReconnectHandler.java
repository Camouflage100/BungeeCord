...
public class YamlReconnectHandler extends AbstractReconnectHandler {
    ...
    @SuppressWarnings("unchecked") public YamlReconnectHandler() {
        try {
            file.createNewFile();
    ...
    @Override protected ServerInfo getStoredServer(ProxiedPlayer player) {
    ...
    @Override public void setServer(ProxiedPlayer player) {
        lock.writeLock().lock();
        try {
            data.put(key(player), (player.getReconnectServer() != null) ?

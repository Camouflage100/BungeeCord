...
public class CommandServer extends Command implements TabExecutor {
    ...
    @Override public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(ProxyServer.getInstance()
                .getTranslation("current_server", player.getServer().getInfo().getName()));
            TextComponent serverList =
                new TextComponent(ProxyServer.getInstance().getTranslation("server_list"));
            ...
            for (ServerInfo server : servers.values()) {
                if (server.canAccess(player)) {
                    ...
                    serverTextComponent.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND,
    ...
    public Iterable<String> onTabComplete(final CommandSender sender, final String[] args) {
        return (args.length > 1) ?
            ...
            Iterables.transform(Iterables.filter(ProxyServer.getInstance().getServers().values(),
                new Predicate<ServerInfo>() {
                    ...
                    @Override public boolean apply(ServerInfo input) {
                        return input.getName().toLowerCase().startsWith(lower) && input
                            .canAccess(sender);
                    }
